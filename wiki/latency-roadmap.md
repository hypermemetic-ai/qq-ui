...
| F — stable prompt replacement | Landed in `1bfdb6d`; **field-rejected** | Prompt geometry and duplicate optimistic/authoritative representations | Durable identity was unavailable until POST completion, leaving queue truth and the provisional visibly duplicated during slow flush. Deterministic response-time cleanup did not satisfy live acceptance. |
| G — correlation identity from birth | Landed in qq-core `31dfdf3` and qq-ui `aac7749`; field-accepted | Queue-before-POST duplicate interval | Browser v4 UUID metadata is captured on the real HTMX request and queue-shaped provisional. Safe correlation on pending/user/steering authority removes the provisional in the queue mutation microtask; durable core message ID remains an exact old-backend fallback. |
| H — queued-to-admitted visual continuity | Implemented here; operator visual verdict pending | Prompt width, origin, wrapping, and height across lifecycle authority changes | Optimistic, accepted/waiting, steering, editable, and admitted prompts use one full-width rectangular block geometry. Real Chromium desktop/Android proofs gate each transition at ≤1px and retain per-frame exact-identity uniqueness. |
