...
| G — correlation identity from birth | Implemented in this change; coordinated core propagation and QA/live retest pending | Queue-before-POST duplicate interval | Browser v4 UUID metadata is captured on the real HTMX request and queue-shaped provisional. Safe correlation on pending/user/steering authority removes the provisional in the queue mutation microtask; durable core message ID remains an exact old-backend fallback. |
