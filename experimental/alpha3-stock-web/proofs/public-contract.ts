...
/** Compile-only proof of the supported alpha.3 contribution calls used by the spike. */
export function registerPublicContributions(ctx: Context): readonly (() => void)[] {
  ...
  const theme = ctx.theme.overrideTokens("qq.alpha3-spike.type-proof", QQ_THEME_TOKENS);
