...
assert.ok(dshPins.length >= 8, "expected an explicit alpha.3 public package set");
...
assert.equal(hostPlugin.name, "qq-ui-alpha3-spike");
...
assert.equal((hostPatch.match(/name: '@hypermemetic-ai\/qq-ui-alpha3-spike'/gu) ?? []).length, 1);
...
assert.equal(plugin.name, "qq-ui-alpha3-spike-client");
...
console.log(`alpha3 stock-module composition proof passed: one ${packageJson.name} factory, sha256:${digest}`);
