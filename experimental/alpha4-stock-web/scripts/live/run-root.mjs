...
const PREFIX = "/tmp/qq-alpha4-live-";
const ROOT_PATTERN = /^\/tmp\/qq-alpha4-live-[A-Za-z0-9][A-Za-z0-9._-]*$/u;
...
export function disposableRootForPath(pathArg) {
  ...
  const match = path.match(/^\/tmp\/qq-alpha4-live-[A-Za-z0-9][A-Za-z0-9._-]*(?=\/|$)/u);
  if (match === null) throw new Error(`path is outside /tmp/qq-alpha4-live-*: ${path}`);
  return match[0];
}
