...
if (!(["disable", "restore"].includes(mode)) || sourceArg === undefined || backupArg === undefined) {
  console.error("usage: node scripts/live/mutate-client.mjs disable|restore <installed-client.js> <backup>");
  process.exit(2);
}
...
if (mode === "restore") {
  ...
  console.log("restored exact installed QQ client artifact");
} else {
  ...
  const indent = original.slice(lineStart, start);
  assert.match(indent, /^\s*$/u, "apply marker must be preceded only by line indentation");
  const endMarker = `\n${indent}};`;
  ...
  const disabled = `${original.slice(0, start)}exports.apply = function apply() {\n${indent}  // Live-gate disposal probe: intentionally no registrations.${original.slice(end)}`;
