...
const scratch = await mkdtemp(join(tmpdir(), "qq-alpha4-type-gate-"));
...
try {
  ...
  assert.match(missing.stderr, /BLOCKED public alpha\.4 type check/u);
  assert.ok(missing.stderr.includes(`${missingName} (missing; expected 0.1.2-alpha.4)`));
...
console.log("alpha4 public-type dependency gate proof passed: missing/mismatch block, exact set invokes tsc");
